...
字典資料在 `data/dictionary/pack/` 下，每個 `.txt` 為 JSON 行格式，內含多個詞條。若發現詞條錯字、錯誤翻譯或符號（如卦象 ☱☶ 對調），可依下列流程**提交 PR** 修正：

1. **指出詞條與錯誤**：在 Cursor / 支援 @ 的編輯器中，可 `@pack` 或直接開啟 `data/dictionary/pack/` 下檔案，說明要訂正的詞與正確內容。例如：「咸」條目寫成「艮（☱）下兌（☶）上」應改為「艮（☶）下兌（☱）上」。
2. **搜尋詞條位置**：詞條可能以 Unicode 跳脫鍵名存在，可用 repo 內搜尋該詞或錯誤字串以定位到對應的 pack 檔案與行。
3. **只改資料、送 PR**：訂正時僅修改 `data/dictionary/pack/*.txt` 內對應的 JSON 行，完成後依「提交程式碼」流程送出 Pull Request。合併後若專案有部署 R2，需重新上傳字典資料（見下方「先上傳資料」）或執行 `commands/upload_dictionary.sh`，線上站才會反映變更。

適合交給 AI 訂正的類型包括：釋義錯字、多語翻譯錯誤、專有名詞或符號對應錯誤（如易經卦象）、引用典籍的筆誤等；若涉及版權或大規模改寫，請先於 issue 討論。
