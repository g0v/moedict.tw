...
describe("/{lang}/index.json (issue #124)", () => {
  it.each(["a", "t", "h", "c"])("returns the word list array for lang=%s", async (lang) => {
    const { status, body } = await fetchJson<string[] | { error: string }>(`/${lang}/index.json`);
...
describe("/api/xref/{lang}.json", () => {
  it.each(["a", "t", "h", "c"])(
    "returns an xref object (or {} fallback) for lang=%s",
    async (lang) => {
...
describe("/api/xref-by-id/{lang}.json", () => {
  it.each(["a", "t", "h", "c"])(
    "returns an xref-by-id object (or {} fallback) for lang=%s",
    async (lang) => {
...
describe("/api/search-index/{lang}.json", () => {
  it.each(["a", "t", "h", "c"])(
    "returns array data for lang=%s when fixture present",
    async (lang) => {
