...
async function setInputValueWithEvent(
  input: Locator,
  value: string,
): Promise<void> {
...
test.describe('mobile sidebar search toggle', () => {
  ...
  test('steps back through mobile search input history before browser history', async ({ page }) => {
    await page.goto('/');
  ...
  test('does not count unfinished IME composition text in mobile search history', async ({ page }) => {
    await page.goto('/%E8%90%8C');
...
test.describe('desktop sidebar search controls', () => {
  ...
  test('steps back through completed search terms without leaving the sidebar layout', async ({ page }) => {
    await page.goto('/');
