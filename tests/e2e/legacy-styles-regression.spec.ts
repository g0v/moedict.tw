...
async function blockCssSubresources(page: Page): Promise<void> {
  ...
  await page.route("**/assets/fonts/**", (route) => {
    ...
    // block those so networkidle can fire.
...
test.describe("legacy styles.css \u2014 zero-regression differential", () => {
  ...
  for (const { name, goto, readiness } of PAGES) {
    test(`${name}: working tree styles.css renders identically to ${BASELINE_REF}`, async ({
      ...
      expect(
        before.length,
        `${name}: computed-style digest must not be empty (sanity check on the digest mechanism itself)`,
      ...
      expect(
        after,
        `${name}: working-tree styles.css must produce an IDENTICAL computed-style digest to ${BASELINE_REF} (post-cascade resolved values for every element, incl. ::before/::after content) — any diff means some rule's effective value changed`,
      ).toBe(before);
    });
  }
});
...
test.describe("legacy styles.css \u2014 harness self-verification (controls)", () => {
  ...
  test("positive control: legacy CSS measurably changes computed styles vs. no CSS at all", async ({
    page,
  }) => {
    ...
    console.log(
      `[positive-control] ${diffs.length} named-property diffs:\n${diffs.map((d) => "  " + d).join("\n")}`,
    );
    expect(
      diffs.length,
      `legacy CSS must measurably change at least one named computed style vs. blank CSS. diffs: ${diffs.join(", ")}`,
    ).toBeGreaterThan(0);
    ...
    expect(
      styledDigest,
      "the full computed-style digest must also differ between blank and real CSS",
  ...
  test("negative control: a deliberately mutated stylesheet changes both the screenshot and the computed-style digest", async ({
    ...
    expect(
      !mutated.equals(original),
      "mutated stylesheet (red body bg) must produce a visibly different screenshot than the original",
    ).toBe(true);
    ...
    expect(
      mutatedDigest,
      "the computed-style digest must also change for a mutated stylesheet",
