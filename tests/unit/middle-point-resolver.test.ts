...
describe('resolveMiddlePointTarget', () => {
  it('maps the four dictionary-entry prefixes', () => {
    expect(resolveMiddlePointTarget('/萌')).toEqual({ page: 'dict', lang: 'a', word: '萌', idx: undefined });
    expect(resolveMiddlePointTarget("/'食")).toEqual({ page: 'dict', lang: 't', word: '食', idx: undefined });
    expect(resolveMiddlePointTarget('/:字')).toEqual({ page: 'dict', lang: 'h', word: '字', idx: undefined });
    expect(resolveMiddlePointTarget('/~上訴')).toEqual({ page: 'dict', lang: 'c', word: '上訴', idx: undefined });
  });

  it('carries the /N definition-index permalink to dictionary pages', () => {
    expect(resolveMiddlePointTarget('/萌/3')).toEqual({ page: 'dict', lang: 'a', word: '萌', idx: 3 });
    expect(resolveMiddlePointTarget("/'%E6%8F%A4/2")).toEqual({ page: 'dict', lang: 't', word: '揤', idx: 2 });
  });

  it('maps radical detail routes', () => {
    expect(resolveMiddlePointTarget('/@木')).toEqual({ page: 'radical', lang: 'a', radical: '木' });
    expect(resolveMiddlePointTarget('/~@水')).toEqual({ page: 'radical', lang: 'c', radical: '水' });
  });

  it('maps starred routes with optional entry suffix', () => {
    expect(resolveMiddlePointTarget('/=*')).toEqual({ page: 'starred', lang: 'a', entry: undefined });
    expect(resolveMiddlePointTarget("/'=*")).toEqual({ page: 'starred', lang: 't', entry: undefined });
  ...
  it('maps group/category list routes', () => {
    expect(resolveMiddlePointTarget('/=成語')).toEqual({ page: 'list', lang: 'a', category: '成語' });
    expect(resolveMiddlePointTarget("/'=動物")).toEqual({ page: 'list', lang: 't', category: '動物' });
    expect(resolveMiddlePointTarget('/~=同實異名')).toEqual({ page: 'list', lang: 'c', category: '同實異名' });
  });
