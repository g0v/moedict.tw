...
describe('renderEmbedDocument', () => {
  it('renders title, pronunciation, and grouped definitions for lang=a', () => {
    ...
    const html = renderEmbedDocument({ word: '萌', lang: 'a', entry, canonicalUrl: 'https://www.moedict.tw/萌' });
  ...
  it('falls back to the pinyin/trs field for pronunciation when bopomofo is absent', () => {
    ...
    const html = renderEmbedDocument({ word: '食', lang: 't', entry, canonicalUrl: "https://www.moedict.tw/'食" });
  ...
  it('skips the pronunciation line entirely for lang=h', () => {
    const entry: EmbedDictionaryEntry = {
      title: '字',
      heteronyms: [{ bopomofo: 'should-not-appear', pinyin: 'ii', definitions: [{ type: '名', def: '文字。' }] }],
    };
    const html = renderEmbedDocument({ word: '字', lang: 'h', entry, canonicalUrl: 'https://www.moedict.tw/:字' });
  ...
  it('drops definitions whose def text is empty after stripping tags', () => {
    const entry: EmbedDictionaryEntry = {
      title: '萌',
      heteronyms: [{ definitions: [{ type: '名', def: '<br>' }, { type: '名', def: '   ' }] }],
    };
    const html = renderEmbedDocument({ word: '萌', lang: 'a', entry, canonicalUrl: 'https://www.moedict.tw/萌' });
  ...
  it('caps heteronyms at 3 and definitions per heteronym at 5', () => {
    ...
    const html = renderEmbedDocument({ word: '多音字', lang: 'a', entry, canonicalUrl: 'https://www.moedict.tw/多音字' });
  ...
  it('strips markup from the title instead of rendering it live, and escapes stray non-tag markup in definitions', () => {
    const entry: EmbedDictionaryEntry = {
      title: '<b>萌</b>',
      heteronyms: [{ definitions: [{ type: '名', def: '<img src=x onerror=alert(1)>惡意 & 危險 < 5' }] }],
    };
    const html = renderEmbedDocument({ word: '萌', lang: 'a', entry, canonicalUrl: 'https://www.moedict.tw/萌' });
