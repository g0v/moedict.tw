...
describe('handleOEmbedAPI', () => {
  it('405s for non-GET/HEAD methods', async () => {
    ...
    const { request, url } = makeRequest('/api/oembed?url=https://www.moedict.tw/萌', { method: 'POST' });
  ...
  it('404s for a url on a disallowed host', async () => {
    const env = makeMengEnv();
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://evil.example.com/萌')}`);
    const res = await handleOEmbedAPI(request, url, env);
  ...
  it('404s for a non-http(s) scheme like ftp:', async () => {
    const env = makeMengEnv();
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('ftp://moedict.tw/萌')}`);
    const res = await handleOEmbedAPI(request, url, env);
    expect(res.status).toBe(404);
  });
  ...
  it('404s for a non-default explicit port', async () => {
    const env = makeMengEnv();
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw:8443/萌')}`);
    const res = await handleOEmbedAPI(request, url, env);
    expect(res.status).toBe(404);
  });
  ...
  it('404s for a moedict.tw url that is not an embeddable entry (e.g. /about)', async () => {
    const env = makeMengEnv();
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw/about')}`);
    const res = await handleOEmbedAPI(request, url, env);
    expect(res.status).toBe(404);
  });
  ...
  it('404s when the entry has no packed data', async () => {
    ...
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw/萌')}`);
    const res = await handleOEmbedAPI(request, url, env);
    expect(res.status).toBe(404);
  });
  ...
  it('501s for format=xml (unsupported, per oEmbed spec)', async () => {
    const env = makeMengEnv();
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw/萌')}&format=xml`);
    const res = await handleOEmbedAPI(request, url, env);
  ...
  it('returns a tokenless oEmbed 1.0 rich payload for www.moedict.tw and moedict.tw hosts', async () => {
    for (const host of ['www.moedict.tw', 'moedict.tw']) {
      const env = makeMengEnv();
      const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent(`https://${host}/萌`)}`);
      const res = await handleOEmbedAPI(request, url, env);
  ...
  it('returns an empty body for HEAD requests', async () => {
    const env = makeMengEnv();
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw/萌')}`, { method: 'HEAD' });
    const res = await handleOEmbedAPI(request, url, env);
    expect(res.status).toBe(200);
  ...
  it('falls back to route.text as the title when the entry has no title field', async () => {
    const env = {
      DICTIONARY: makeR2({
        'pack/12.txt': JSON.stringify({ [escape('萌')]: { h: [{ d: [{ f: '無題定義。', type: '名' }] }] } }),
      }),
    };
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw/萌')}`);
    const res = await handleOEmbedAPI(request, url, env);
  ...
  it('falls back to route.text when the entry title is present but strips down to nothing', async () => {
    const env = {
      DICTIONARY: makeR2({
        'pack/12.txt': JSON.stringify({ [escape('萌')]: { t: '   ', h: [{ d: [{ f: '空白標題。', type: '名' }] }] } }),
      }),
    };
    const { request, url } = makeRequest(`/api/oembed?url=${encodeURIComponent('https://www.moedict.tw/萌')}`);
    const res = await handleOEmbedAPI(request, url, env);
