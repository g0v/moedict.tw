...
- **CNS（`/api/cns/*`）刻意不版本化**：全字庫是政府釋出、少再生（`UPLOAD_SCOPE=cns`，~77k 物件、數小時上傳）。不建 `cns-corpus/current.json`；edge 用 bare URL + `Cache-Tag: cns,cns-record`，上傳後以 tag purge 翻新。ingredient（`data/sources/cns-data-manifest.json` 的 `output_checksum`）已在，若再生變頻繁或要 mid-upload 一致再加 pointer。
