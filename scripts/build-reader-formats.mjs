...
function renderDefinition(definition) {
  ...
  const examples = Array.isArray(definition?.e)
    ? definition.e.map(nonEmptyHtml).filter(Boolean)
    : [];
...
function renderHeteronym(heteronym, index) {
  ...
  if (definitions.length > 0) {
    const items = definitions.map(
      (definition, definitionIndex) => `${definitionIndex + 1}. ${renderDefinition(definition)}`,
    );
...
function renderEntryHtml(headword, lang, entry) {
  ...
  const translation =
    entry?.translation && typeof entry.translation === "object" && !Array.isArray(entry.translation)
      ? entry.translation
      : {};
  const translations = [
    [
  ...
  if (heteronyms.length > 0) {
    blocks.push(
      heteronyms.map((heteronym, index) => renderHeteronym(heteronym, index)).join("<br><br>"),
    );
  }
...
function renderMobiChunkHtml(lang, records, chunkIndex, chunkCount) {
  ...
  const items = records
    .map(
      (record) =>
        `<hr /><h2>${escapeHtml(record.word)}</h2><p>${renderXhtmlFragment(record.article)}</p>`,
    )
    .join("\n");
...
function renderOpf(lang, baseName, chunkFiles) {
  ...
  const manifestItems = chunkFiles
    .map(
      (file, index) =>
        `<item id="chunk-${index}" href="${file}" media-type="application/xhtml+xml" />`,
    )
    .join("\n    ");
...
async function writeMobiFiles(lang, records, converter) {
  ...
  const recordsPerChunk =
    Number.isFinite(MOBI_RECORDS_PER_CHUNK) && MOBI_RECORDS_PER_CHUNK > 0
  ...
  if (converter.type === "ebook-convert") {
    ...
    await executeCommand(converter.command, [opfPath, "-o", `${baseName}.mobi`], {
      label: `${converter.type} (${lang})`,
    });
  } else {
    await executeCommand(converter.command, [opfPath, mobiPath], {
      label: `${converter.type} (${lang})`,
    });
  }
