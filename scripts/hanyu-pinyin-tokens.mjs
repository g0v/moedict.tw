...
export function analyzePinyinField(value, lang = "a") {
  ...
  let tokens = stripped.split(/([a-z]+)/i).filter((token) => /^[a-z]/.test(token));
  ...
  if (lang === "c") {
    ...
    tokens = tokens.flatMap((token) => token.split(syllableSplit)).filter(Boolean);
  }
