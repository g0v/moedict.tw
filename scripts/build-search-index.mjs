...
async function buildLangIndex(lang, sourceDirName) {
  ...
  for (const bucketFile of bucketFiles) {
    ...
    for (const [packedKey, entry] of Object.entries(bucket)) {
      ...
      const content = dedupeStrings(
        collectSearchStrings(entry).filter((part) => part !== title),
      ).join(" ");
  ...
  console.log(
    `[build-search-index] wrote ${lang}: ${items.length} entries (${formatBytes(Buffer.byteLength(payload))})`,
  );
}
