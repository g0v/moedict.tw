...
function main() {
  ...
  for (const { name, entries } of cats) {
    ...
    catWritten++;
    if (name === '成語') {
      console.log(`成語 ${entries.length} -> ${filtered.length} (removed ${entries.length - filtered.length})`);
    }
  }
  ...
  console.log(`@人 flat=${flat.length} missingFromIndex=${missingRen.length}`, missingRen.slice(0, 5));
