...
> **目前狀態：** 零停機實作分支仍在 Task 4 與 staging／production 出貨完成前的最終整合中；截至本日**尚未宣稱 production 已採用本流程**。Task 4 完成後，請用本文的「出貨前檢查」更新狀態。
...
- design 與任務報告：[`docs/superpowers/specs/2026-07-12-zero-downtime-release-design.md`](../docs/superpowers/specs/2026-07-12-zero-downtime-release-design.md)、`.superpowers/sdd/task-1-report.md`、`task-2-report.md`、`task-3-report.md`。
...
- [ ] `package.json` 的 `deploy`／`deploy:staging` 已 clean cutover 至 `node scripts/release-deploy.mjs`，沒有標準命令可走裸 `wrangler deploy`。
- [ ] `deploy:rollback`、`deploy:publish-only` 的語意與本文一致，且使用 generated config。
- [ ] `AGENTS.md`、`README.md`、`docs/superpowers/recovery.md` 已同步此流程與 positional recovery 命令。
- [ ] `wrangler.jsonc` top-level 與 staging 都有正確 `version_metadata` binding。
- [ ] staging 已以實際 preview Worker/R2 完成 build → publish/verify → two-phase rollout → 120 秒 probe → final smoke。
- [ ] production gate 核對同一 git SHA + 同一 manifest digest，並以 production 自己重建 digest 再驗一次。
- [ ] production rollout、final smoke、headers/logs 與 R2 fallback 已在 `www.moedict.tw` 實際觀察；將本文開頭「尚未採用」改為含日期、release ID、version UUID 的事實紀錄。
- [ ] 保留 deployment state、probe 結果、manifest digest 與 rollback 結果；不要在沒有證據時宣稱零停機。
