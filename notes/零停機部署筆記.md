...
- [x] `package.json` 的 `deploy`／`deploy:staging` 已 clean cutover 至 `vp run build && node scripts/release-publish.mjs && node scripts/release-deploy.mjs`（staging 每段各帶 `CLOUDFLARE_ENV=staging`），沒有標準命令可走裸 `wrangler deploy`；由 `tests/unit/package-deploy-scripts.test.ts` 與 CI 的 `scripts/check-deploy-scripts-safety.mjs` 鎖定。
