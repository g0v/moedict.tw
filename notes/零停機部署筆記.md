...
> **目前狀態：** Task 4（config／scripts／docs 全整合）已完成。**現行 production（2026-07-13 06:45:16Z）**：release `23b7e89-1d1f2400cb1d`（stroke-corpus + propagation sleep，含完整 6,063 筆 stroke-json），version UUID `2be488db-8ad7-4384-bc2a-c539b4196445` @100%。**注意：** `c7aa7d03-8bce95414453`（#150 edge cache 真實啟用 + R2 memoize，finalized 2026-07-13T03:12:33Z）**早於** `23b7e89` 出貨（03:12 vs 06:45），已被後者覆蓋——目前 live production **不含** edge cache fix（`cf-cache-status: HIT` 尚未生效）。`c7aa7d0` 本次 merge 後已納入 integration，下次出貨後才會真正生效。`www.moedict.tw` 現跑 `23b7e89` 路徑。
