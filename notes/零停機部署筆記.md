...
> **目前狀態：** Task 4（config／scripts／docs 全整合）已完成。**Staging 已於 2026-07-13 重新出貨至 origin/main tip（0a2599b）並完成 audit**。**Production 已於 2026-07-13 採用零停機協議完成出貨**：release `0a2599b-5db141ad4cd4`，version UUID `5699a0f8-e741-41ca-90d8-323fbe8de41c`，finalized `2026-07-12T23:31:20.799Z`（UTC）。`www.moedict.tw` 正在跑新路徑。
...
- [x] staging 已以實際 preview Worker/R2 完成 build → publish/verify → two-phase rollout → 120 秒 probe → final smoke。**事實紀錄（第一次 2026-07-12）：** release ID `8fc64f2-5db141ad4cd4`；version UUID `b5d0fc27-7372-4607-8232-d07abe6c3a3b`；finalized `2026-07-12T13:11:27.704Z`。**重新出貨（2026-07-13，origin/main tip 0a2599b）：** release ID `0a2599b-5db141ad4cd4`；version UUID `9c5952d7-951e-4d4f-a712-88d2c64e1d0e`；worker `cf-moedict-webkit-neo-staging` @ 100%（finalized `2026-07-12T23:23:08.228Z`）；R2 bucket `moedict-assets-preview` key prefix `releases/0a2599b-5db141ad4cd4/`（45 files + manifest，authenticated re-GET OK）；live probes `/` `/about` `/api/config` `/api/%E8%90%8C.json` `/assets/index-BJFTx66N.js` `/assets/index-BKH8HGTI.css` 皆 200 且 `X-Moedict-Release: 0a2599b-5db141ad4cd4` + `X-Moedict-Version: 9c5952d7-951e-4d4f-a712-88d2c64e1d0e`；shell `X-Moedict-Shell-Source: site-assets`；assets `X-Moedict-Asset-Source: site-assets`；local state `.wrangler/releases/staging/current.json` + `staging-approval.json`（`gitSha: "0a2599b"`, `clientManifestDigest: "5db141ad4cd4"`, `approvedAt: "2026-07-12T23:23:08.228Z"`）保存於 ship worktree。
- [x] production gate 核對同一 git SHA + 同一 manifest digest，並以 production 自己重建 digest 再驗一次。**事實紀錄（2026-07-13）：** staging approval `gitSha: "0a2599b"` + `clientManifestDigest: "5db141ad4cd4"`；production build 於同一 worktree 同一 commit（`0a2599b`）產生相同 digest `5db141ad4cd4`；gate 通過，無需人工介入。
- [x] production rollout、final smoke、headers/logs 與 R2 fallback 已在 `www.moedict.tw` 實際觀察。**事實紀錄（2026-07-13）：** release ID `0a2599b-5db141ad4cd4`；version UUID `5699a0f8-e741-41ca-90d8-323fbe8de41c`；worker `cf-moedict-webkit-neo` @ 100%（finalized `2026-07-12T23:31:20.799Z`）；舊 version `e5955708` 已完全移除（deployment list 唯一一筆 `5699a0f8@100%`）；R2 bucket `moedict-assets` key prefix `releases/0a2599b-5db141ad4cd4/`（45 files + manifest，authenticated re-GET OK）；live probes `/` `/about` `/api/config` `/api/%E8%90%8C.json` `/assets/index-BJFTx66N.js` `/assets/index-BKH8HGTI.css` 皆 200 且 `X-Moedict-Release: 0a2599b-5db141ad4cd4` + `X-Moedict-Version: 5699a0f8-e741-41ca-90d8-323fbe8de41c`；shell `X-Moedict-Shell-Source: site-assets`；2 分鐘連續 curl（23 次）零 4xx/5xx；headless browser 6 次（4 desktop + Android + iPhone UA）零 console.error／pageerror／failed-request；直接導覽 `/語言` 標題正確顯示「語言 - 萌典」。
- [x] 保留 deployment state、probe 結果、manifest digest 與 rollback 結果（staging 與 production 雙側）。**staging 與 production 證據已保存**於 ship worktree `.wrangler/releases/staging/`、`.wrangler/releases/production/` 與 `staging-approval.json`；`.superpowers/sdd/task-5-prod-report.md` 保存完整 production audit 證據。
