...
> **適用日期：** 2026-07-12（staging，第一次上線）/ 2026-07-13（production，第二、三次上線）/ 2026-07-18–19（staging，PR #152）。\
> **目的：** 給值班操作者與維護者的可執行說明。這份筆記是 `notes/重要路由筆記.md` 同等重要、但更完整的部署運行手冊。  
> **目前狀態（2026-07-19 更新，已核實）：** **production 自 2026-07-13T06:45:16Z 起未再變動**——仍是 `23b7e89-1d1f2400cb1d`（`fix/full-moe-stroke-corpus` 分支程式碼：6,063 筆全量 stroke-json pipeline + propagation sleep），version UUID `2be488db-8ad7-4384-bc2a-c539b4196445` @100%。**`23b7e89` 分支不包含 `c7aa7d0`（#150 edge-cache 真實啟用 + R2 memoize，finalized 2026-07-13T03:12:33Z）**——`git merge-base --is-ancestor` 雙向核實兩者互不為對方祖先，是各自獨立的分支；`c7aa7d0` 只進了 `main`（經 `d887339`），從未併入 `23b7e89` 這條出貨線。換言之，**production 現在仍然沒有 #150 的 edge cache 修復**（`X-Moedict-Edge-Cache` 標頭、`isEdgeCacheable`／`caches.default` 讀寫層目前只存在 `main`／PR #152 程式碼中，尚未進 production）。
>
...
>
> **下一次 production rollout 會重新引入 #150**：因為 production 目前跑的是 `23b7e89`（不含 #150），而任何從 `main`（含 `2898dd7`／PR #152，也含 `c7aa7d0`／#150）出貨到 production 的下一次部署，都會同時帶入 #150 的 edge-cache 修復——這是預期內、非本次修復刻意引入的副作用，值得部署前的 checklist 明確記錄，避免 operator 誤以為 #150 是全新變更而重新驗證一次已經驗證過的東西。
