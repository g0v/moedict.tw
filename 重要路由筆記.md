...
**2026-07 更新**：筆畫 JSON 已透過
`commands/migrate-legacy-cdn-to-r2.mjs` 遷移至 Cloudflare R2
（`https://r2-assets.moedict.tw/stroke-json/`）。四本字典的發音音檔
目前仍由上方 3 個 Rackspace 主機提供；R2 `audio/` 物件尚未完整，
因此現行程式碼在 `src/utils/media-cdn.ts` 刻意讓 `AUDIO_CDN_MAP`
維持 legacy hosts。本檔的音檔 http-map 仍是現行路由來源；第 5、6 列
筆畫歷史主機未納入目前路由。
