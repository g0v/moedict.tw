  ...
  if (/DT$/.test(system)) {
    ...
    if (isBody) {
      converted = converted.replace(
        /((?:[^.,!?]*(?:\w[^-.,!?\w\s\u2011]*)[- \u2011])+)(\w)/g,
        (_, prefix: string, tail: string) =>
          prefix
            .split(/([- \u2011.,!?])/)
            .map((seg) => toneSandhi(seg))
            .join("") + tail,
      );
    } else {
      converted = converted.replace(
...
const V_RE = new RegExp(
  Object.keys(TAIWANESE_VOWELS)
    .sort((a, b) => b.length - a.length)
    .join("|"),
  "g",
);
const CV_RE = new RegExp(
  `^(${Object.keys(TAIWANESE_CONSONANTS)
    .sort((a, b) => b.length - a.length)
    .join("|")})((?:${Object.keys(TAIWANESE_VOWELS)
    .sort((a, b) => b.length - a.length)
    .join("|")})+[ptkh]?)$`,
);
...
function placeTlToneMark(syllable: string, mark: string): string {
  const insertAt = (pos: number) =>
    `${syllable.slice(0, pos + 1)}${mark}${syllable.slice(pos + 1)}`;
...
export function trsToBpmf(lang: Lang, trs: string): string {
  ...
  return source
    .replace(/(?:[A-Za-z]|[\u0300-\u030D])+/gu, (chunk) => {
      ...
      token = token.replace(
        CV_RE,
        (_, consonant: string, rest: string) => `${TAIWANESE_CONSONANTS[consonant]}${rest}`,
      );
