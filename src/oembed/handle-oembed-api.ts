...
function oembedResponse(method: string, payload: unknown, status: number, cacheTag?: string): Response {
...
export async function handleOEmbedAPI(request: Request, url: URL, env: OEmbedEnv): Promise<Response> {
  ...
  if (format !== 'json') {
    return oembedResponse(request.method, { error: 'not_implemented', message: `format=${format} 尚未支援，僅提供 json。` }, 501);
  }

  const targetUrlParam = url.searchParams.get('url');
  if (!targetUrlParam) {
    return oembedResponse(request.method, { error: 'bad_request', message: '缺少必要參數 url。' }, 400);
  }
  ...
  try {
    ...
    return oembedResponse(request.method, { error: 'bad_request', message: 'url 參數不是合法網址。' }, 400);
  }
  ...
  if (
    ...
    return oembedResponse(request.method, { error: 'not_found', message: '僅支援 moedict.tw 詞條網址。' }, 404);
  }

  const route = parseDictionaryRoute(targetUrl.pathname);
  if (!route || !route.text) {
    return oembedResponse(request.method, { error: 'not_found', message: '此網址不是可嵌入的詞條頁面。' }, 404);
  }

  const entry = (await lookupDictionaryEntry(route.text, route.lang, env)) as EmbedDictionaryEntry | null;
  ...
  const height = clampToRange(url.searchParams.get('maxheight'), DEFAULT_HEIGHT, MIN_HEIGHT, MAX_HEIGHT);
