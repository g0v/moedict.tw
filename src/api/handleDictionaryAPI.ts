  ...
  if (text === "index") {
    ...
    if (!indexObject) {
      return jsonResponse(
        ...
        {
          error: "Not Found",
          message: `找不到索引檔：${lang}/index.json`,
          terms: [],
...
function processDictionaryEntry(
  ...
  if (parsedEntry.English || parsedEntry.english)
    result.English = parsedEntry.English || parsedEntry.english;
  ...
  if (parsedEntry.non_radical_stroke_count)
    result.non_radical_stroke_count = parsedEntry.non_radical_stroke_count;
...
function decodeLangPart(lang: DictionaryLang, part = ""): string {
  ...
  part = part.replace(
    /([\u4e00-\u9fff])\u20DD/g,
    '<span class=\\"regional part-of-speech\\">$1</span> ',
  );
