...
function DropdownSubmenu({
  ...
  const enterDropdown: MouseEventHandler<HTMLLIElement> = (e) => {
    ...
    const innerDropdownMenu = e.currentTarget.querySelector<HTMLUListElement>(
      `.${styled.dropdownMenu}`,
    );
    ...
    currentSiblings
      ...
      .forEach((el) => el.classList.remove(styled.pinned));
  ...
  return (
    <li
      ...
      onClick={pinDropdown}
    >
      <a href="#" className={`${lang} taxonomy`}>
        {item.label}
      </a>
...
export function NavbarNormal({ currentLang }: NavbarNormalProps) {
  ...
  const handleLangOptionClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>, toLang: Lang) => {
      ...
      >
        ...
        <ul className="nav navbar-nav">
          ...
          <li>
            ...
            <ul
              role="navigation"
              className={`${styled.dropdownMenuRoot}`}
              ref={dropdownMenuRootRef}
            >
            ...
            <Link to="/about" title="關於本站" aria-label="關於本站">
              <span className="iconic-circle" style={{ backgroundColor: "#400" }}>
                <SvgIcon
                  name="info"
                  size="1em"
                  className={styled.navCircleIcon}
                  aria-hidden="true"
                />
              </span>
            </Link>
